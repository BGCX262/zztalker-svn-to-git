...
public:
    ...
    void destroyRoster();  /*注销账户*/
    bool passChange(QString oldPass, QString newPass);     /*改变密码*/
    bool transFile(QString toJid, QString fileName, QString desc, int size);  /*传输文件*/
...
signals:
    ...
    void destroyRosterSuccess();
    void passChangeSuccess();
    //准备好接收文件,三个参数分别为 来源jid/文件名/文件大小/会话id
    void readyReceiveFile(QString, QString, int, QString);
    void readySentFile();      //准备好发送文件
    void beginSentFile();      //开始发送
    /*开始接受文件，参数按顺序为 jid/主机地址/主机端口/sid/会话id*/
    void beginReceiveFile(QString, QString,QString,QString,QString);
...
private:
    ...
    bool parseQueryItem(QTreeWidgetItem *item, QString type, QString sessionId);
    bool parsePresenceItem(QTreeWidgetItem *item);
    bool parseMessageItem(QTreeWidgetItem *item);
    bool parseSiItem(QTreeWidgetItem *item, QString fromJid);
