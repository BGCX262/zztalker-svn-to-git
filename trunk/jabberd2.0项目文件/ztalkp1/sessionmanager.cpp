...
/*处理query节*/
bool SessionManager::parseQueryItem(QTreeWidgetItem *item, QString type, QString id)
{
    QMap<QString, QString> attrs = getAttrList(item);
    QString xmlns = attrs["namespace"];
    QString sid = attrs["sid"];
    if(xmlns=="http://jabber.org/protocol/bytestreams")
    {
        for(int i=0; i<item->childCount();i++)
        {
            QTreeWidgetItem *childItem = item->child(i);
            if(childItem->text(0) == "streamhost-used")
            {
                status = BEGINSENT;
                return true;
            }else if(childItem->text(0) == "streamhost"){
                QMap<QString, QString> servAtts = getAttrList(childItem);
                QString host = servAtts["host"];
                QString port = servAtts["port"];
                QString jid = servAtts["jid"];
                status = BEGINRECEIVE;
                emit beginReceiveFile(jid, host, port, sid, id);
                return true;
            }
        }
    }
    ...
    if(type == "subscribe")
    {
        ...
        for(int i=0; i<listCount; i++)
        {
            ...
            if(fri["jid"] == jid)
            {
                ...
                friendsList[i]["resource"] = friendPre["from"].split("/").at(1);
            }
        }
    }
...
QString SessionManager::generateResponse()
{
    ...
    case INITIALIZED:
        ...
        if(retryTime++ >= 3)
        {
            ...
            errString = QString("SASL Authorization failed more than 3 times.\nLogin failed.");
    ...
    case PASSCHANGESUCCESS:
        emit passChangeSuccess();
        break;
    case DESTROYROSTERSUCCESS:
        emit destroyRosterSuccess();
        break;
    case MESSAGE:
        break;
    case ADDITEM:
...
void SessionManager::destroyRoster()
{
    status = DESTROYROSTER;
    QString response = QString("<iq type='set' from='%1' id='unreg1'>"
...
bool SessionManager::passChange(QString oldPass, QString newPass)
{

    if(user["password"].trimmed() != oldPass)
    {
        errString = QString("Old password is wrong.");
        return false;
    }

    if(newPass.trimmed().length()==0)
    {
        errString = QString("New password can not be null.");
        return false;
    }

    QString response = QString("<iq type='set' to='%1' id='change1'>"
                                "<query xmlns='jabber:iq:register'>"
                                "<username>%2</username>"
                                "<password>%3</password>"
                                "</query></iq>")
...
bool SessionManager::transFile(QString toJid, QString fileName, QString desc, int size)
{
    int index = fileName.lastIndexOf("/");
    QString shortName = fileName.mid(index+1);

    srand(time(0));
    sid="s55_";
    for(int k=0; k<16; k++)
    {
        sid += QString::number(random()%16, 16);
    }

    QString request = QString("<iq type=\"set\" to=\"%1\" id=\"aac3a\">"
                              "<si xmlns=\"http://jabber.org/protocol/si\" "
                              "profile=\"http://jabber.org/protocol/si/profile/file-transfer\" "
                              "id=\"%2\">"
                              "<file xmlns=\"http://jabber.org/protocol/si/profile/file-transfer\" "
                              "size=\"%3\" name=\"%4\">"
                              "<desc>%5</desc><range/></file>"
                              "<feature xmlns=\"http://jabber.org/protocol/feature-neg\">"
                              "<x xmlns=\"jabber:x:data\" type=\"form\">"
                              "<field type=\"list-single\" var=\"stream-method\">"
                              "<option><value>http://jabber.org/protocol/bytestreams</value>"
                              "</option></field></x></feature></si></iq>")
                              .arg(toJid).arg(sid).arg(size).arg(shortName).arg(desc);
    emit send(request);
}
